...
class Arrow:
    ...
    def __init__(self, R1, R2, bottom, top, t):
        ...
        bottom : np.ndarray of shape (3,)
            Shaft base point.
        top : np.ndarray of shape (3,)
            Cone apex point.
        t : float
    ...
    def mesh(self):
        ...
        cylinder_mesh = Cylinder(bottom=self.bottom, top=start_cone, radius=self.R1).mesh()
...
class Disk:
    ...
    def __init__(self, center,normal_vector, R):
        ...
        center : array-like of shape (3,)
            Disk center.
        normal_vector : np.ndarray of shape (3,)
            Plane normal.
        R : float
            Disk radius.
        """
...
class PipeSurface:
    ...
    def __init__(self, curve, Range, R):
        ...
        ----------
        curve : callable
            Function ``t -> np.ndarray`` returning curve positions.
    ...
    def tangent_vector(self, t):
        ...
        -------
        np.ndarray
            Finite-difference tangent vector.  Normalize it before using it as
            a direction vector.
        """
    ...
    def mesh(self, caps = False, resolution_circle = 50, resolution_time = 100):
        ...
        ----------
        caps : bool, optional
            When ``True`` add disk caps at the start and end of the curve.
        resolution_circle : int, optional
            Number of samples around each circular cross-section and cap.
        resolution_time : int, optional
            Number of samples along the guiding curve.
        ...
        if caps == False:
            ...
            tapa_abajo = Disk(center = self.curve(self.Range[0]), normal_vector=-self.tangent_vector(self.Range[0]), R = self.R).mesh_data(num_points=resolution_circle)
            tapa_arriba = Disk(center = self.curve(self.Range[1]), normal_vector=self.tangent_vector(self.Range[1]), R = self.R).mesh_data(num_points=resolution_circle)
            mesh_tapa_abajo = trimesh.Trimesh(vertices = tapa_abajo[0], faces = tapa_abajo[1])
...
class Cylinder:
    ...
    sections : int, optional
        Number of radial segments passed to ``trimesh.creation.cylinder`` (default ``64``).
    ...
    def export_mesh(self, name, caps=False):
        ...
        name : str or path-like
            Output file path.
        caps : bool, optional
            When ``True`` export ``delaunay_mesh(caps=True)``. Otherwise export
            the procedural mesh from :meth:`mesh`.
        """
        if caps:
            self.delaunay_mesh(caps=True).export(name)
        else:
            self.mesh().export(name)
...
class RuledSurface:
    ...
    def pointcloud(self, s_resolution = 100, t_resolution = 100):
        ...
        Parameters
        ----------
        s_resolution : int, optional
            Number of interpolation samples between the two curves.
        t_resolution : int, optional
            Number of samples along each bounding curve.
    ...
    def mesh_data(self, s_resolution = 100, t_resolution = 100):
        ...
        # Get number of points per curve sample
        num_points_per_circle = s_resolution
...
class Torus:
    ...
    def __init__(self, center, axis, R_major, R_minor, major_sections=32, minor_sections=32):
        ...
        R_major : float
            Radius from the torus center to the tube centerline.
        R_minor : float
            Radius of the tube.
        major_sections : int, optional
            Number of samples around the torus centerline.
        minor_sections : int, optional
            Number of samples around the tube cross-section.
        """
        self.center = center
        self.axis = axis
        self.R_major = R_major
        self.R_minor = R_minor
        self.major_sections = int(major_sections)
        self.minor_sections = int(minor_sections)
        if self.major_sections < 3 or self.minor_sections < 3:
            raise ValueError("major_sections and minor_sections must be >= 3")
    ...
    def mesh(self):
        ...
        torus = trimesh.creation.torus(
            major_radius=self.R_major,
            minor_radius=self.R_minor,
            major_sections=self.major_sections,
            minor_sections=self.minor_sections,
        )
