r""" Point Milling Library

This script provides the essential methods regarding point milling with a
toroidal/flat-end cutter. Actual computations are carried out on the offset,
allowing us to simplify both the math and the code.
...
def get_coefficients_of_cubic_at_z(X, z):
    r'''Returns the coefficients of a Bezier cubic curve whose control points are given by X and we want to solve for z
...
def roots_of_cubic_equation(X):
    ...
    ----------
    X : array-like
        An array or list of coefficients for the cubic polynomial. The coefficients
        are expected in the order of decreasing powers, i.e., [a, b, c, d] for ax^3 + bx^2 + cx + d.

    Returns
    -------
    np.ndarray
        An array containing the real roots of the cubic polynomial. Complex roots are ignored,
...
def get_coordinates(V,V1,V2):
    ...
    Notes
    -----
    The function requires that V, V1, and V2 have compatible dimensions. If V.ndim is 2, then V1 and V2 must also be 2D with the same number of vectors as V or be broadcastable to that shape.
    """
...
def du_dv(surface, X):
    r"""Gets function and point and returns the vector of the derivatives
    ...
    np.ndarray
        ...
        is the derivative at the corresponding point.
    """
...
def normal(surface, X):
    r"""Gets function and point and returns the normal vector to the
...
def duu_duv(surface, x):
    r"""Gets function and point and returns the second derivatives d^2/du^2, d^2/dudv
...
def dvv(surface, x):
    r"""Gets function and point and returns the second derivatives d^2/dv^2
    of surface at point(s) X. 
...
def check_plane_side(point_to_check, normal_vector, point_in_plane):
    r"""
    Determines the position of one or more points relative to corresponding planes in 3D space. Each plane is defined by a normal vector and a point on the plane. The function evaluates the dot product between each plane's normal vector and the vector from the plane's point to the point to be checked. 

    Parameters
    ----------
    point_to_check : np.ndarray
        An array of points to be checked. This can be a single point (as a 1D array of shape (3,)) or multiple points (as a 2D array where each row represents a point in 3D space).
    normal_vector : np.ndarray
        The normal vectors of the planes. Should be a single vector (as a 1D array of shape (3,)) if checking one point or multiple vectors (as a 2D array with each row as a vector) corresponding to each point if multiple points are checked.
    point_in_plane : np.ndarray
        Points that lie on each plane. Should be a single point (as a 1D array of shape (3,)) if checking one point or multiple points (as a 2D array where each row represents a point) corresponding to each normal vector if multiple points are checked.
...
class BezierPatch:
    r"""
    A class to create and evaluate a bicubic Bézier surface patch, assuming the definition domain is the unit square [[0,1], [0,1]]. This class is used to compute the surface point and derivatives at given parameter values.
...
class Surface:
    r"""
    A class that encapsulates a differentiable surface defined over R^2 ---> R^3, using a Bézier bicubic patch representation for practical computation. This surface is defined by a control net which determines its shape, and provides functionalities to compute various geometric properties such as derivatives, normals, and point clouds.

    Attributes
    ----------
    function : callable
        A function that maps R^2 to R^3, representing the surface.
    du_dv_n : callable
    ...
    def point_cloud(self, n_points_u = 50, n_points_v = 50):
        ...
        -------
        tuple
            A list containing the point coordinates and a triangulation
            object for plotting, as well as vertex normal vectors.
        """
        ...
        vertex_normals = self.normal_vector(np.array([u,v]).T)
        return [[x,y,z], tri, vertex_normals]
...
class Envelope:
    r"""
    Represents the envelope of a medial circle in motion over a surface. This class models the behavior
    of a toolpath envelope in machining processes with flat-end cutting or toroidal cutters.
    ...
    def value_at(self, s,t, method = 'point'):
        ...
        if  method == 'point':
            ...
            B = np.cross(normal,tangent)*(cosF*(cosT - cosS*cosT) + sinF*sinS) 
            ...
            B = np.cross(normal,tangent)*(cosF*(cosT - cosS*cosT) + sinF*sinS)[:, np.newaxis]
    ...
    def shank_at_t(self, t, distance = 1+np.sqrt(5) ):
        ...
        if np.isscalar(t):
            ...
            d_orto = -sinF*tangent + cosF*np.cross(normal,tangent)
            ...
            d_orto = -sinF[:, np.newaxis]*tangent + cosF[:, np.newaxis]*np.cross(normal,tangent)
    ...
    def cylinder_at_t(self, t, distance = 1+np.sqrt(5)):
        """
...
class G_function:
    """
    Class representing the G-function defined over a Bézier patch surface. This class provides methods to compute
    level sets of the G-function, which are useful in various applications such as contour tracing and intersection finding.
